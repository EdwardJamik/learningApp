...
export default function LevelPage() {
	...
	const levelId = params.level; // Отримуємо ID рівня з URL (a1, a2, тощо)
	console.log(params)
	...
	// Підрахунок кількості модулів та уроків
	const totalModules = levelData.modules.length;
	const totalLessons = levelData.modules.reduce((sum, module) => sum + module.lessons.length, 0);
			...
			<section className="current-level">
				...
				<div className="container">
					<div className="header-section">
						<div className={`lesson-item ${levelData.id}`}>
						...
						<div className="progress-card">
							<div className="progress-info">
								<h2 className="progress-title">
									{totalModules} розділів / {totalLessons} занять
								</h2>
								<p className="progress-subtitle">
									<b>Завершено 0</b> / {totalLessons} занять
								</p>
							</div>
							<button className="btn btn-primary btn-large">
								Купити повний доступ до курсу
							</button>
						</div>
					</div>
					...
					<div className="lessons-list">
						{levelData.modules.map((module, index) => (
							<Link
								key={module.id}
								href={`/${locale !== 'ru' ? `level/${levelId}/${module.id}` : `ru/level/${levelId}/${module.id}`}`}
							>
								<div className="lesson-card">
									...
									<div className="lesson-actions">
										{/* Можна додати умову для відображення кнопки "Куплено" */}
